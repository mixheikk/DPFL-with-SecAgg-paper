...
class SimpleRngClientManager(ClientManager):
    """Provides a pool of available clients. Uses explicit numpy rng to avoid randomness issues with Ray."""
    ...
    def get_one_client(self, criterion: Optional[Criterion] = None) -> ClientProxy:
        """Return any available client. Avoids evolving internal RNG state."""
        available_cids = list(self.clients)
        if criterion is not None:
            available_cids = [
                cid for cid in available_cids if criterion.select(self.clients[cid])
        ...
        return self.clients[available_cids[0]]
    ...
    def sample(
        ...
        #sampled_cids = random.sample(available_cids, num_clients)
        sampled_cids = list(self.rng.choice(available_cids, num_clients, replace=False, shuffle=True))
        #print('CM sampled', sampled_cids, 'from', available_cids, type(sampled_cids))
