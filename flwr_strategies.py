...
from flwr.server.strategy.aggregate import simple_average as aggregate_simple_average
...
# flake8: noqa: E501
class FedAvg(FedAvg):
    """Configurable FedAvg with Momentum implementation, combined with model checkpointing, and robust aggregation functions."""
    ...
    def __init__(
        ...
        *,
        ...
        self.last_server_round = last_server_round
        self.aggregate = lambda x: aggregate_simple_average(x)
    ...
    def configure_fit(
        ...
        config['n_sampled_clients'] = sample_size # add number of sampled clients to config
    ...
    def aggregate_fit(
        ...
        if not self.accept_failures and failures:
            raise ValueError(f'Failures not accepted, but got failures: {failures}')
        weights_results = [
            (parameters_to_ndarrays(fit_res.parameters), fit_res.num_examples)
            for _, fit_res in results
        ]

        fedavg_result = self.aggregate((weights_results, parameters_to_ndarrays(self.initial_parameters) ))
        # initialise previous iteration weights to 0s if not present
        if self.initial_parameters is None:
            tmp = []
            for w in fedavg_result:
                tmp.append(np.zeros_like(w))
            self.initial_parameters = ndarrays_to_parameters(tmp)

        # originally following convention described in
